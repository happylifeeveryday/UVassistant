  ...
  <div class="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4">
    <div class="max-w-xl w-full bg-white rounded-lg shadow-md p-8 relative">
      ...
      <div class="grid grid-cols-1 gap-6">
        <div>
          ...
          <div class="relative">
            <select v-model="lastApplied" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-transparent">
        ...
        <div>
          ...
          <div class="relative">
            <select v-model="spfValue" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-transparent">
        ...
        <div>
          ...
          <div class="relative">
            <select v-model="skinTone" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-transparent">
              <option v-for="tone in skinTones" :key="tone" :value="tone">{{ tone }}</option>
            </select>
            <i class="fas fa-user-circle absolute right-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
          </div>
        </div>
        ...
        <button @click="setReminder" class="w-full bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 transition flex items-center justify-center gap-2">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-lg">
        ...
        <button @click="closeModal" class="mt-4 w-full bg-gray-500 hover:bg-gray-600 text-white py-2 rounded-lg">
          Close
        </button>
      </div>
    </div>
